...
- Все цвета интерфейса и задач — CSS-переменные на `<html data-theme>`; CSS генерируется из одного файла данных `lib/themes.ts` (он же рисует превью в настройках). Смена схемы — смена атрибута, без ререндера React.
- **Тушь** (по умолчанию): «Тушь», «Тушь с пометками», «Суми» (тёмная). Почти монохром: бумага, тушь, тонкая нить; единственный цвет — киноварь у текущего блока (`--c-now`). В «пометках» иконка тонирована тёмным приглушённым цветом категории. «Как в системе» = пометки днём, Суми ночью.
- **Цветные**: Молоко, Бумага, Туман, Матча, Графит, Чернила. У каждой — своя вручную подобранная семёрка цветов в её характере (Туман: клюква, тауп, хаки, эвкалипт, сталь, сланец; Бумага: сиена, умбра, охра, олива…), заливка капсулы — этот цвет, растворённый в поверхности/фоне схемы, иконка — притемнённая текстом схемы. Проверено скриптом: соседние цвета различимы, контраст иконки к заливке ≥ 3.9:1.
- Схема может задать: `now` — общий цвет «сейчас» (иначе — цвет задачи через `var(--c-now, …)`; остальные схемы явно сбрасывают его в `initial`, чтобы киноварь не протекла из `:root`), `onSolid` — цвет иконки на заливке (тёмный в тёмных схемах), толщину нити, а у каждого цвета — `mark` для пикеров: капсулы «Туши» одного тона, а выбор категории остаётся различимым.

...
- Схема задаёт весь интерфейс (фон, поверхности, 4 уровня текста, линии, нить, акцент, цвет удаления) **и свои тона капсул**. «Молоко» и «Графит» — эталоны с ручными тонами; остальные выводятся из них под температуру схемы: оттенок подтягивается к тону схемы (не больше 10°, противоположные цвета только приглушаются — иначе охра в «Тумане» ушла бы в салатовый), заливка растворяется в поверхности схемы, иконка — в её цвете текста. Проверено: соседние цвета различимы не хуже эталона, контраст иконки к заливке ≥ 3.5:1.
